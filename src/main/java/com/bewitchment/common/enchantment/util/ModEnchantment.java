...
public class ModEnchantment extends Enchantment
{
	...
	public List<ItemStack> getEntityEquipment(EntityLivingBase living)
	{
		...
		if (living instanceof EntityPlayer)
		{
			...
			for (int i = 0; i < handler.getSlots(); i++) if (!handler.getStackInSlot(i).isEmpty()) result.add(handler.getStackInSlot(i));
		}
		return result;
	}
	...
	{
		int max = 0;
		for (ItemStack stack : getEntityEquipment(player)) if (EnchantmentHelper.getEnchantmentLevel(this, stack) > max) max = EnchantmentHelper.getEnchantmentLevel(this, stack);
		return max;
	}
