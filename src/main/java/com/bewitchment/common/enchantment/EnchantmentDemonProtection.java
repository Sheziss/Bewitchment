...
public class EnchantmentDemonProtection extends ModEnchantment {
	...
	public void applyEnchantment(LivingHurtEvent event, int level) {
		if (event.getSource().getTrueSource() instanceof EntityLivingBase && ((EntityLivingBase) event.getSource().getTrueSource()).getCreatureAttribute() == BewitchmentAPI.SPIRIT)
