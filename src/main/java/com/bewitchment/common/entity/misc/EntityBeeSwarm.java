...
public class EntityBeeSwarm extends Entity
{
	...
	public void onUpdate()
	{
		...
		if (ticksExisted == 1) for (int i = 0; i < 512; i++) Bewitchment.proxy.spawnParticle(ModParticles.BEE, posX - 0.5 + rand.nextDouble(), posY - 0.5 + rand.nextDouble(), posZ - 0.5 + rand.nextDouble());
		if (ticksExisted % 3 == 0) world.playSound(null, getPosition(), ModSounds.BUZZ, SoundCategory.BLOCKS, 1, (float) (1 + (rand.nextGaussian() * 0.05)));
	}
