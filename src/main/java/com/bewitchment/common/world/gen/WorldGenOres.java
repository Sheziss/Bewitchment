...
public class WorldGenOres implements IWorldGenerator {
	...
	public void generate(Random rand, int chunkX, int chunkZ, World world, IChunkGenerator generator, IChunkProvider provider) {
		if (world.provider instanceof WorldProviderSurface) // for (int j = 0; j < 32; j++) // uncomment this for mega ores
		{
			...
			for (int i = 0; i < Bewitchment.proxy.config.alexandriteChance; i++)
				alexandrite.generate(world, rand, new BlockPos(chunkX * 16 + rand.nextInt(16), rand.nextInt(Bewitchment.proxy.config.alexandriteMax - Bewitchment.proxy.config.alexandriteMin) + Bewitchment.proxy.config.alexandriteMin, chunkZ * 16 + rand.nextInt(16)));
			...
			for (int i = 0; i < Bewitchment.proxy.config.bloodstoneChance; i++)
				bloodstone.generate(world, rand, new BlockPos(chunkX * 16 + rand.nextInt(16), rand.nextInt(Bewitchment.proxy.config.bloodstoneMax - Bewitchment.proxy.config.bloodstoneMin) + Bewitchment.proxy.config.bloodstoneMin, chunkZ * 16 + rand.nextInt(16)));
			...
			for (int i = 0; i < Bewitchment.proxy.config.jasperChance; i++)
				jasper.generate(world, rand, new BlockPos(chunkX * 16 + rand.nextInt(16), rand.nextInt(Bewitchment.proxy.config.jasperMax - Bewitchment.proxy.config.jasperMin) + Bewitchment.proxy.config.jasperMin, chunkZ * 16 + rand.nextInt(16)));
			for (int i = 0; i < Bewitchment.proxy.config.malachiteChance; i++)
				malachite.generate(world, rand, new BlockPos(chunkX * 16 + rand.nextInt(16), rand.nextInt(Bewitchment.proxy.config.malachiteMax - Bewitchment.proxy.config.malachiteMin) + Bewitchment.proxy.config.malachiteMin, chunkZ * 16 + rand.nextInt(16)));
			for (int i = 0; i < Bewitchment.proxy.config.nuummiteChance; i++)
				nuummite.generate(world, rand, new BlockPos(chunkX * 16 + rand.nextInt(16), rand.nextInt(Bewitchment.proxy.config.nuummiteMax - Bewitchment.proxy.config.nuummiteMin) + Bewitchment.proxy.config.nuummiteMin, chunkZ * 16 + rand.nextInt(16)));
			for (int i = 0; i < Bewitchment.proxy.config.tigersEyeChance; i++)
				tigersEye.generate(world, rand, new BlockPos(chunkX * 16 + rand.nextInt(16), rand.nextInt(Bewitchment.proxy.config.tigersEyeMax - Bewitchment.proxy.config.tigersEyeMin) + Bewitchment.proxy.config.tigersEyeMin, chunkZ * 16 + rand.nextInt(16)));
			for (int i = 0; i < Bewitchment.proxy.config.tourmalineChance; i++)
				tourmaline.generate(world, rand, new BlockPos(chunkX * 16 + rand.nextInt(16), rand.nextInt(Bewitchment.proxy.config.tourmalineMax - Bewitchment.proxy.config.tourmalineMin) + Bewitchment.proxy.config.tourmalineMin, chunkZ * 16 + rand.nextInt(16)));
		}
	}
}