...
public class BlockCropBelladonna extends ModBlockCrop {
	...
	public void updateTick(World world, BlockPos pos, IBlockState state, Random rand) {
		...
		if (isMaxAge(state) && rand.nextInt(10) <= 3) {
			...
			if (world.getBlockState(pos0.down()).getBlock().canSustainPlant(state, world, pos, EnumFacing.UP, this) && getSeed().soil.contains(world.getBlockState(pos0.down()).getBlock()) && world.isAirBlock(pos0))
