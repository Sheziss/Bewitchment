...
public class BlockCropKenaf extends ModBlockCrop {
	...
	public void grow(World world, BlockPos pos, IBlockState state) {
		if (isMaxAge(state) && world.isAirBlock(pos.up()) && !state.getValue(TOP))
			world.setBlockState(pos.up(), world.getBlockState(pos.down()).getBlock() == this ? getDefaultState().withProperty(TOP, true) : getDefaultState());
		else
			world.setBlockState(pos, getDefaultState().withProperty(AGE, Math.min(state.getValue(AGE) + getBonemealAgeIncrease(world), getMaxAge())).withProperty(TOP, state.getValue(TOP)));
	}
