...
public class BlockSaltBarrier extends ModBlock {
	...
	public void addCollisionBoxToList(IBlockState state, World world, BlockPos pos, AxisAlignedBB box, List<AxisAlignedBB> boxes, Entity entity, boolean wut) {
		...
		if (entity instanceof EntityBlaze || entity instanceof EntityGhast || entity instanceof EntityVex /* || entity instanceof EntityBatSwarm */)
			addCollisionBoxToList(pos, box, boxes, WALL);
		if (entity instanceof EntityPlayer && !((EntityPlayer) entity).isCreative() && !entity.getCapability(ExtendedPlayer.CAPABILITY, null).getTransformation().canCrossSalt)
			addCollisionBoxToList(pos, box, boxes, WALL);
	}
