...
public class EventHandler {
	...
	public void livingUpdate(LivingEvent.LivingUpdateEvent event) {
		if (event.getEntityLiving().world.getBlockState(event.getEntityLiving().getPosition()).getBlock() == ModObjects.fluid_honey.getBlock())
		...
		if (event.side == Side.SERVER && event.phase == Phase.START) {
			...
			if (world.getTotalWorldTime() % 20 == 0 && BiomeDictionary.hasType(world.getBiome(event.player.getPosition()), Type.FOREST)) {
				...
				if (world.provider.getDimension() == 0 && world.provider.getMoonPhase(world.getWorldTime()) == 4 && !world.isDaytime() && rand.nextDouble() < 0.2) {
					...
					for (int i = -5; i <= 5; i++) {
						pos.setY(y + i);
						if ((world.isAirBlock(pos) || world.getBlockState(pos).getBlock().isReplaceable(world, pos)) && world.getBlockState(pos.down()).getBlock() == Blocks.DIRT)
